...
--d)Como já deveriamos saber fazer

select f.nome
from Funcionario f, Pedido pe, Itens i, Produto pr
where f.codigo = pe.vendedor and pe.codigo = i.pedido and pr.codigo = i.produto and pr.venda = (select min(venda) from Produto)
...
select nome 
from Funcionario
where codigo in (select vendedor from Pedido
				 where codigo in 
				(select pedido from Itens
				 where produto in
				(select codigo from Produto
				 where venda = (select min (venda) from Produto))))
...
where pr.venda = (select min (venda) from Produto)

--i) Qual o nome do cliente que mais realizou pedidos no ano de 2018?

select cl.nome, count(*) as quantPedidos
INTO tabResult 
from Cliente cl JOIN Pedido pe on pe.cliente = cl.codigo 
where DATEPART(yy,pe.dataPedido) = 2019
group by cl.nome


select nome 
from tabResult 
where quantPedidos = ( select max(quantPedidos) from tabResult)