...
--a) Crie uma Stored Procedure que, passando como parâmetro de entrada o código do funcionário, mostre todas
--as datas dos pedidos que foram realizados por este funcionário.


CREATE Procedure ListaDatasPedFunc
@codfuncionario int as
select datapedido
from Pedido
where vendedor = @codfuncionario

exec ListaDatasPedFunc 2

--b) Crie uma Stored Procedure, que passando como parâmetro de entrada o código do pedido, mostre o nome do
--cliente que fez este pedido, o nome e o preço de venda do produto comprado e a data do pedido.

Create Procedure CliPed	
@codPedido int as 
select c.nome, p.nome, p.venda, pe.dataPedido
from Cliente c, Itens i, Produto p, Pedido pe
where c.codigo = pe.cliente and p.codigo = i.produto and pe.codigo = i.pedido and p.codigo = @codPedido  

exec CliPed 2

select * FROM Pedido

--c) Crie uma Stored Procedure que mostre para cada funcionário passado como parâmetro de entrada, o nome da
--cidade e do país que ele nasceu. Passe como parâmetro de entrada o nome do funcionário.
...
CREATE PROCEDURE QuantPedAno

@nomeCli varchar(80), @anoPedido int, @quantPed int OUTPUT as
select @quantPed = count(*)

from Cliente c JOIN Pedido pe on c.codigo = pe.cliente
where c.nome = @nomeCli and DATEPART(yy, pe.dataPedido) = @anoPedido


DECLARE @quantPed int 
exec QuantPedAno 'Larissa Maia', 2020, @quantPed output 
select @quantPed


--e) Crie uma Stored Procedure que retorne como parâmetro de saída o nome do funcionário mais velho.

alter Procedure NomeFuncionarioVelho
@nomeFunc varchar(80) output as 

select @nomeFunc = nome
from Funcionario
where datanasc = (select Min(datanasc) from Funcionario)

declare @nomeFunc varchar (80) 
exec NomeFuncionarioVelho @nomeFunc output
select @nomeFunc