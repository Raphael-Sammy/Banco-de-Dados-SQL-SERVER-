...
--os produtos.
--b) Mostre a média de salários dos funcionários.
--c) Mostre todos os pedidos realizados no ano de 2009.
--d) Exibir os nomes das distintas cidades e bairros dos funcionários da empresa.
--e) Mostre quantos pedidos foram realizados
--f) Mostre quantos funcionários tem e-mail
--g) Exibir código e nome de todos os funcionários que tenham “Silva” em qualquer parte do nome
--h) Mostre os produtos comprados por clientes pessoa jurídica.
--i) Mostre os nomes e as cidades que nasceram os funcionários que realizaram pedidos de Liquidificador.
--j) Exiba nome e tipo dos clientes que foram atendidos por funcionários que não moram no Bessa nem em
--Manaíra.
--k) Mostre os nomes dos funcionários que realizaram algum pedido para clientes que moram no Rio de
--Janeiro.

--l) Mostre nome, descrição e data da compra dos produtos comprados entre 2020 e 2022. Ordene pela
--data em ordem descendente.
--m) Mostre os nomes dos clientes que informaram o cargo, e que realizaram alguma compra de pizza em
--2017.
--n) Mostre para cada funcionário o seu nome e a quantidade de vendas realizadas.
--o) Mostre o nome do produto, o seu valor de venda, a quantidade vendida e a soma dos preços de venda
...
--p) Exibir o código do produto e a quantidade de pedidos feitos para os produtos que foram pedidos mais
--do que 30 vezes.
...
select p.codigo, p.nome, p.custo, p.venda, (p.venda - p.custo) as diferença
...
select pr.codigo, pr.nome, pr.custo, pr.venda, (venda - custo) as diferenca
from Produto pr, Tipo ti
where pr.codigo = ti.codigo
...
--Resposta C  Mostre todos os pedidos realizados no ano de 2009.

select p.cliente Cliente, p.codigo Codigo, p.dataPedido Data_do_Pedido, p.vendedor Codigo_Vendedor
from Pedido p, Cliente c, Funcionario f
where p.cliente = c.codigo and p.vendedor = f.codigo and p.dataPedido between '2009-01-01' and '2009-12-31'


SELECT P.cliente Cliente, P.codigo Pedido , P.dataPedido DataDoPedido, P.vendedor Vendedor
FROM Pedido P
WHERE P.dataPedido BETWEEN '2009-01-01' AND '2009-12-31';


--Resposta D Exibir os nomes das distintas cidades e bairros dos funcionários da empresa.

select f.nome, c.nome, f.bairro Bairro
from Funcionario f , Cidade c
where f.cidade = c.codigo 


--Resposta E) Mostre quantos pedidos foram realizados

select count(*) as Total_Pedidos_Realizados
...
--Resposta F Mostre quantos funcionários tem e-mail

select count(*) as Quant_Func_Com_Email
from Funcionario f
where f.email is not null;


--Resposta G Exibir código e nome de todos os funcionários que tenham “Silva” em qualquer parte do nome
...
--Resposta H Mostre os produtos comprados por clientes pessoa jurídica.
